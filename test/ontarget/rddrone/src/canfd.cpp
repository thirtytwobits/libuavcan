...
volatile static std::uint32_t data_ISR_word[libuavcan::media::S32K::InterfaceGroup::FrameType::MTUBytes / 4u];
...
public:
    ...
    virtual ~S32KInterfaceGroupImpl()
    {
        ...
        for (std::uint8_t i = 0; i < CANFD_Count; i++)
        {
            ...
            while (FlexCAN[i]->MCR, CAN_MCR_LPMACK_MASK)
                ;
    ...
    void isrHandler(std::uint8_t instance)
    {
        ...
        if (mb_index)
        {
            ...
            if (frame_ISRbuffer_[instance].size() <= Frame_Capacity)
            {
...
private:
     ...
     */
    Result messageBuffer_Transmit(std::uint_fast8_t iface_index, std::uint8_t TX_MB_index, const FrameType& frame) const
    {
         ...
         * timeout for it */
        Result Status = flagPollTimeout_Set(FlexCAN[iface_index]->IFLAG1, 1u << TX_MB_index);
